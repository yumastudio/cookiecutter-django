...
MEDIA_PATH = "/media/"
MEDIA_URL = "https://api.{{cookiecutter.project_slug}}.dev" + MEDIA_PATH
...
def get_aliases(aspects, widths):
    return {
        "aspect-%s-width-%s" % (aspect['name'], width): {
            'size': (width, width*aspect['ratio']),
            'crop': True,
            'aspect': aspect['name']
        } for aspect in aspects for width in widths
    }


THUMBNAIL_ALIASES = {
    'users.User.avatar': get_aliases(
        aspects=USER_ASPECTS,
        widths=[48, 140])
}
THUMBNAIL_EXTENSION = "jpeg"
THUMBNAIL_NAMER = "config.namers.alias"
